// Auto-generated from the Pigsfield Excel sheets by tools/convert.py
// Safe to edit by hand — this is the live data of the website.
window.PF_DATA = window.PF_DATA || {};
window.PF_DATA.tools = {
 "sections": [
  {
   "title": "🛠️ Pigsfield — Tools with Tutorials | 71 Tools · 6 Categories · 100% Free",
   "groups": [
    {
     "title": "AI & ML — Best AI tools + direct tutorials",
     "items": [
      {
       "title": "ChatGPT",
       "desc": "लॉजिक समझने, कोडिंग करने, और जनरल सवालों के जवाब के लिए दुनिया का सबसे बेहतरीन AI। Pro: $20/month",
       "links": [
        {
         "label": "Web",
         "urls": [
          "https://chat.openai.com/",
          "https://chatgpt.com/codex/"
         ]
        },
        {
         "label": "App",
         "urls": [
          "https://play.google.com/store/apps/details?id=com.openai.chatgpt"
         ]
        },
        {
         "label": "Tutorial",
         "urls": [
          "https://www.youtube.com/results?search_query=how+to+use+chatgpt+full+tutorial+course"
         ]
        }
       ]
      },
      {
       "title": "Claude AI",
       "desc": "लॉजिक, कोडिंग और बड़े डॉक्यूमेंट्स एनालाइज करने में मास्टर। जवाब नियंत्रित और ethical हैं।",
       "links": [
        {
         "label": "Web",
         "urls": [
          "https://claude.ai/",
          "https://claude.ai/code/"
         ]
        },
        {
         "label": "App",
         "urls": [
          "https://play.google.com/store/apps/details?id=com.anthropic.claude"
         ]
        },
        {
         "label": "Tutorial",
         "urls": [
          "https://www.youtube.com/results?search_query=claude+ai+full+tutorial+use+cases"
         ]
        }
       ]
      },
      {
       "title": "Claude Artifacts",
       "desc": "Claude के अंदर ही UI डिज़ाइन, कोड, और इंटरैक्टिव ऐप्स जनरेट करने का फ्रंटएंड साथ में।",
       "links": [
        {
         "label": "Web",
         "urls": [
          "https://claude.ai/design"
         ]
        },
        {
         "label": "Tutorial",
         "urls": [
          "https://www.youtube.com/results?search_query=claude+artifacts+tutorial"
         ]
        }
       ]
      },
      {
       "title": "Google Gemini",
       "desc": "रियल-टाइम वेब सर्च और गूगल के ईकोसिस्टम (Docs, Drive, Gmail) के साथ इंटीग्रेट AI।",
       "links": [
        {
         "label": "Web",
         "urls": [
          "https://gemini.google.com/",
          "https://antigravity.google/"
         ]
        },
        {
         "label": "App",
         "urls": [
          "https://play.google.com/store/apps/details?id=com.google.android.apps.bard"
         ]
        },
        {
         "label": "Tutorial",
         "urls": [
          "https://www.youtube.com/results?search_query=how+to+use+google+gemini+tutorial"
         ]
        }
       ]
      },
      {
       "title": "chat.z.ai",
       "desc": "💰 100% फ्री AI — GLM 5.2 मॉडल जो ChatGPT, Claude और Gemini जैसी पेड AI से मुकाबला करता है।",
       "links": [
        {
         "label": "Web",
         "urls": [
          "https://chat.z.ai/"
         ]
        },
        {
         "label": "Tutorial",
         "urls": [
          "https://www.youtube.com/results?search_query=chat+z+ai+tutorial+how+to+use"
         ]
        }
       ]
      },
      {
       "title": "Google AI Studio",
       "desc": "डेवलपर्स के लिए गूगल का प्लेग्राउंड — बिना लिमिट के Gemini के एडवांस्ड मॉडल्स टेस्ट करो।",
       "links": [
        {
         "label": "Web",
         "urls": [
          "https://aistudio.google.com/"
         ]
        },
        {
         "label": "Tutorial",
         "urls": [
          "https://www.youtube.com/results?search_query=google+ai+studio+tutorial"
         ]
        }
       ]
      },
      {
       "title": "Be AI Updated (YouTube)",
       "desc": "AI और टेक्नोलॉजी पर बेस्ट YouTube क्रिएटर्स",
       "links": [
        {
         "label": "Tutorial",
         "urls": [
          "https://youtube.com/@asliengineering",
          "https://youtube.com/@primeventurepartners",
          "https://youtube.com/@gkcs",
          "https://youtube.com/@dwarkeshpatel",
          "https://youtube.com/@machinelearningstreettalk",
          "https://youtube.com/@wolframresearch",
          "https://youtube.com/@varunmayya",
          "https://youtube.com/@cuttingedgeschool"
         ]
        }
       ]
      },
      {
       "title": "NotebookLM",
       "desc": "अपने PDF या नोट्स अपलोड करो — यह सिर्फ उसी डेटा के आधार पर जवाब देता है।",
       "links": [
        {
         "label": "Web",
         "urls": [
          "https://notebooklm.google.com/"
         ]
        },
        {
         "label": "Tutorial",
         "urls": [
          "https://www.youtube.com/results?search_query=google+notebooklm+tutorial"
         ]
        }
       ]
      },
      {
       "title": "Google DeepMind",
       "desc": "गूगल की AI रिसर्च लैब — AlphaFold, Gemini और दुनिया के सबसे एडवांस्ड AI मॉडल्स यहीं बनते हैं।",
       "links": [
        {
         "label": "Web",
         "urls": [
          "https://deepmind.google/"
         ]
        },
        {
         "label": "Tutorial",
         "urls": [
          "https://www.youtube.com/results?search_query=google+deepmind+research+overview"
         ]
        }
       ]
      },
      {
       "title": "Gemma",
       "desc": "गूगल का ओपन-सोर्स हल्का LLM — अपने फोन या लैपटॉप पर लोकली रन करो।",
       "links": [
        {
         "label": "Web",
         "urls": [
          "https://deepmind.google/models/gemma/"
         ]
        },
        {
         "label": "App",
         "urls": [
          "https://play.google.com/store/apps/details?id=com.google.ai.edge.gallery"
         ]
        },
        {
         "label": "Tutorial",
         "urls": [
          "https://www.youtube.com/results?search_query=gemma+model+local+tutorial"
         ]
        }
       ]
      },
      {
       "title": "Google Flow",
       "desc": "AI से सिनेमैटिक वीडियो और फिल्म सीन जनरेट करने वाला गूगल का फिल्ममेकिंग टूल।",
       "links": [
        {
         "label": "Web",
         "urls": [
          "https://labs.google/fx/tools/flow"
         ]
        },
        {
         "label": "Tutorial",
         "urls": [
          "https://www.youtube.com/results?search_query=google+flow+ai+filmmaking+tutorial"
         ]
        }
       ]
      },
      {
       "title": "Google Labs",
       "desc": "गूगल के सभी AI एक्सपेरिमेंट्स और बीटा प्रोजेक्ट्स का सेंट्रल हब।",
       "links": [
        {
         "label": "Web",
         "urls": [
          "https://labs.google/"
         ]
        },
        {
         "label": "Tutorial",
         "urls": [
          "https://www.youtube.com/results?search_query=google+labs+ai+experiments"
         ]
        }
       ]
      },
      {
       "title": "Sarvam AI",
       "desc": "खास तौर पर भारतीय भाषाओं (हिंदी, तमिल आदि) और वॉइस-बेस्ड AI मॉडल्स पर फोकस करता है।",
       "links": [
        {
         "label": "Web",
         "urls": [
          "https://www.sarvam.ai/"
         ]
        },
        {
         "label": "Tutorial",
         "urls": [
          "https://www.youtube.com/results?search_query=sarvam+ai+tutorial+india"
         ]
        }
       ]
      },
      {
       "title": "Uncensored Chat",
       "desc": "कॉरपोरेट फिल्टर-फ्री AI मॉडल्स — ब्रूटली ऑनेस्ट जवाब देते हैं।",
       "links": [
        {
         "label": "Web",
         "urls": [
          "https://uncensored.chat"
         ]
        },
        {
         "label": "Tutorial",
         "urls": [
          "https://www.youtube.com/results?search_query=how+to+use+uncensored+ai+models"
         ]
        }
       ]
      },
      {
       "title": "Opal",
       "desc": "गूगल का एक्सपेरिमेंटल AI प्रोजेक्ट।",
       "links": [
        {
         "label": "Web",
         "urls": [
          "https://opal.google"
         ]
        },
        {
         "label": "Tutorial",
         "urls": [
          "https://www.youtube.com/results?search_query=google+opal+ai+tutorial"
         ]
        }
       ]
      }
     ]
    },
    {
     "title": "Privacy & Web — Privacy tools, browsers, and web utilities",
     "items": [
      {
       "title": "GetIntoPC",
       "desc": "विंडोज़ के भारी प्रीमियम सॉफ्टवेयर (डेवलपमेंट, ग्राफ़िक्स) फ्री में पाओ।",
       "links": [
        {
         "label": "Web",
         "urls": [
          "https://getintopc.com/"
         ]
        },
        {
         "label": "Tutorial",
         "urls": [
          "https://www.youtube.com/results?search_query=how+to+download+from+getintopc+safely"
         ]
        }
       ]
      },
      {
       "title": "Modyolo",
       "desc": "प्रीमियम और मॉडिफाई किए गए Android गेम्स और ऐप्स (Mod APKs) सुरक्षित तरीके से डाउनलोड करो।",
       "links": [
        {
         "label": "Web",
         "urls": [
          "https://modyolo.com/"
         ]
        },
        {
         "label": "Tutorial",
         "urls": [
          "https://www.youtube.com/results?search_query=how+to+download+from+modyolo"
         ]
        }
       ]
      },
      {
       "title": "Tor Browser",
       "desc": "इंटरनेट पर पूरी तरह एनोनिमस रहने के लिए। डेटा को अलग-अलग सर्वर्स से bounce करता है।",
       "links": [
        {
         "label": "Web",
         "urls": [
          "https://www.torproject.org/"
         ]
        },
        {
         "label": "App",
         "urls": [
          "https://play.google.com/store/apps/details?id=org.torproject.torbrowser"
         ]
        },
        {
         "label": "Tutorial",
         "urls": [
          "https://www.youtube.com/results?search_query=tor+browser+tutorial+for+beginners"
         ]
        }
       ]
      },
      {
       "title": "Brave",
       "desc": "Chrome जैसा, लेकिन इन-बिल्ट तगड़े ऐड-ब्लॉकर और ट्रैकर-ब्लॉकर के साथ।",
       "links": [
        {
         "label": "Web",
         "urls": [
          "https://brave.com/"
         ]
        },
        {
         "label": "App",
         "urls": [
          "https://play.google.com/store/apps/details?id=com.brave.browser"
         ]
        },
        {
         "label": "Tutorial",
         "urls": [
          "https://www.youtube.com/results?search_query=brave+browser+best+settings+tutorial"
         ]
        }
       ]
      },
      {
       "title": "Comet Browser",
       "desc": "Browse like your talking to Jarvis of tony stark",
       "links": [
        {
         "label": "App",
         "urls": [
          "https://play.google.com/store/apps/details?id=ai.perplexity.comet"
         ]
        },
        {
         "label": "Tutorial",
         "urls": [
          "https://www.youtube.com/results?search_query=comet+browser+tutorial"
         ]
        }
       ]
      },
      {
       "title": "YT ReVanced",
       "desc": "YouTube का ओपन-सोर्स वर्ज़न — ऐड्स ब्लॉक, बैकग्राउंड प्ले, स्पॉन्सर स्किप सब होता है।",
       "links": [
        {
         "label": "Web",
         "urls": [
          "https://revanced.app/"
         ]
        },
        {
         "label": "Tutorial",
         "urls": [
          "https://www.youtube.com/results?search_query=youtube+revanced+install+tutorial"
         ]
        }
       ]
      },
      {
       "title": "Seal",
       "desc": "डाउनलोडिंग का असली किंग — yt-dlp पर चलता है, ज़ीरो ऐड्स, किसी भी साइट से media निकालो।",
       "links": [
        {
         "label": "Web",
         "urls": [
          "https://github.com/JunkFood02/Seal"
         ]
        },
        {
         "label": "Tutorial",
         "urls": [
          "https://www.youtube.com/results?search_query=seal+video+downloader+tutorial"
         ]
        }
       ]
      },
      {
       "title": "Snaptube",
       "desc": "YouTube, Instagram आदि से वीडियो/ऑडियो डाउनलोड करने का पुराना जुगाड़ (ऐड्स आते हैं)।",
       "links": [
        {
         "label": "Web",
         "urls": [
          "https://www.snaptube.com/"
         ]
        },
        {
         "label": "Tutorial",
         "urls": [
          "https://www.youtube.com/results?search_query=how+to+use+snaptube"
         ]
        }
       ]
      },
      {
       "title": "Stacher",
       "desc": "🎬 किसी भी YouTube चैनल के सभी वीडियो एक ही बार में डाउनलोड करो। TikTok, Instagram से भी।",
       "links": [
        {
         "label": "Web",
         "urls": [
          "https://stacher.io/"
         ]
        },
        {
         "label": "Tutorial",
         "urls": [
          "https://www.youtube.com/results?search_query=how+to+use+stacher+media+downloader"
         ]
        }
       ]
      },
      {
       "title": "e-Raktkosh",
       "desc": "देशभर के ब्लड बैंक्स में खून की रियल-टाइम अवेलेबिलिटी चेक करने का सरकारी ऐप।",
       "links": [
        {
         "label": "Web",
         "urls": [
          "https://eraktkosh.mohfw.gov.in"
         ]
        },
        {
         "label": "App",
         "urls": [
          "https://play.google.com/store/apps/details?id=com.cdac.eraktkosh"
         ]
        },
        {
         "label": "Tutorial",
         "urls": [
          "https://www.youtube.com/results?search_query=eraktkosh+blood+bank+tutorial"
         ]
        }
       ]
      },
      {
       "title": "Have I Been Pwned",
       "desc": "तुम्हारा ईमेल या पासवर्ड हैकर्स के पास लीक तो नहीं हो गया — तुरंत चेक करो।",
       "links": [
        {
         "label": "Web",
         "urls": [
          "https://haveibeenpwned.com"
         ]
        },
        {
         "label": "Tutorial",
         "urls": [
          "https://www.youtube.com/results?search_query=haveibeenpwned+tutorial+check+email+hack"
         ]
        }
       ]
      }
     ]
    },
    {
     "title": "Files & Remote — Cloud, PDFs, file managers, and remote desktop",
     "items": [
      {
       "title": "DigiLocker",
       "desc": "मार्कशीट्स, आधार, ड्राइविंग लाइसेंस डिजिटली और कानूनी रूप से स्टोर करने का भारत का सरकारी तरीका।",
       "links": [
        {
         "label": "Web",
         "urls": [
          "https://www.digilocker.gov.in/"
         ]
        },
        {
         "label": "App",
         "urls": [
          "https://play.google.com/store/apps/details?id=com.digilocker.android"
         ]
        },
        {
         "label": "Tutorial",
         "urls": [
          "https://www.youtube.com/results?search_query=how+to+use+digilocker+app"
         ]
        }
       ]
      },
      {
       "title": "Drive / Docs",
       "desc": "क्लाउड स्टोरेज, वर्ड प्रोसेसिंग और क्विक आइडियाज नोट करने का डिफ़ॉल्ट टूल।",
       "links": [
        {
         "label": "Web",
         "urls": [
          "https://drive.google.com"
         ]
        },
        {
         "label": "Tutorial",
         "urls": [
          "https://www.youtube.com/results?search_query=google+drive+docs+full+tutorial"
         ]
        }
       ]
      },
      {
       "title": "CX File Explorer",
       "desc": "पावरफुल फाइल मैनेजर। फोन से कंप्यूटर या क्लाउड ड्राइव्स (FTP, SMB) को एक्सेस करो।",
       "links": [
        {
         "label": "App",
         "urls": [
          "https://play.google.com/store/apps/details?id=com.cxinventor.file.explorer"
         ]
        },
        {
         "label": "Tutorial",
         "urls": [
          "https://www.youtube.com/results?search_query=cx+file+explorer+smb+ftp+tutorial"
         ]
        }
       ]
      },
      {
       "title": "PDF24 Tools",
       "desc": "PDF का Swiss Army Knife — 100% फ्री, बिना वॉटरमार्क के मर्ज, स्प्लिट, रिसाइज सब।",
       "links": [
        {
         "label": "Web",
         "urls": [
          "https://tools.pdf24.org/"
         ]
        },
        {
         "label": "Tutorial",
         "urls": [
          "https://www.youtube.com/results?search_query=how+to+use+pdf24+tools"
         ]
        }
       ]
      },
      {
       "title": "Windows App",
       "desc": "फोन से दूर रखे Windows PC या Microsoft Cloud PC को कंट्रोल करने के लिए।",
       "links": [
        {
         "label": "App",
         "urls": [
          "https://play.google.com/store/apps/details?id=com.microsoft.windows365.test"
         ]
        },
        {
         "label": "Tutorial",
         "urls": [
          "https://www.youtube.com/results?search_query=microsoft+windows+app+android+tutorial"
         ]
        }
       ]
      },
      {
       "title": "Remote Desktop",
       "desc": "फोन से किसी भी कंप्यूटर की स्क्रीन कंट्रोल करने के लिए गूगल का फ्री टूल।",
       "links": [
        {
         "label": "Web",
         "urls": [
          "https://remotedesktop.google.com"
         ]
        },
        {
         "label": "App",
         "urls": [
          "https://play.google.com/store/apps/details?id=com.google.chromeremotedesktop"
         ]
        },
        {
         "label": "Tutorial",
         "urls": [
          "https://www.youtube.com/results?search_query=chrome+remote+desktop+setup+tutorial"
         ]
        }
       ]
      },
      {
       "title": "Albo",
       "desc": "यूनिवर्सल सेव-फॉर-लेटर ऐप — कोई भी लिंक, वीडियो या आर्टिकल एक जगह सेव कर।",
       "links": [
        {
         "label": "App",
         "urls": [
          "https://play.google.com/store/apps/details?id=com.thesortdapp.sortd"
         ]
        },
        {
         "label": "Tutorial",
         "urls": [
          "https://www.youtube.com/results?search_query=albo+save+for+later+app"
         ]
        }
       ]
      }
     ]
    },
    {
     "title": "Creativity — Video, music, photo tools with tutorials",
     "items": [
      {
       "title": "VideoCompress AI",
       "desc": "AI की मदद से वीडियो की क्वालिटी गिराए बिना उसका साइज (MB) कम करने का टूल।",
       "links": [
        {
         "label": "Web",
         "urls": [
          "https://videocompress.ai/"
         ]
        },
        {
         "label": "Tutorial",
         "urls": [
          "https://www.youtube.com/results?search_query=videocompress.ai+tutorial"
         ]
        }
       ]
      },
      {
       "title": "Suno (AI Music)",
       "desc": "टेक्स्ट में बताओ कैसा गाना चाहिए — वोकल्स और म्यूजिक के साथ स्टूडियो क्वालिटी गाना बनाता है।",
       "links": [
        {
         "label": "Web",
         "urls": [
          "https://suno.com/"
         ]
        },
        {
         "label": "App",
         "urls": [
          "https://play.google.com/store/apps/details?id=com.suno.app"
         ]
        },
        {
         "label": "Tutorial",
         "urls": [
          "https://www.youtube.com/results?search_query=how+to+use+suno+ai+music+generator"
         ]
        }
       ]
      },
      {
       "title": "YT Create",
       "desc": "YouTube का अपना फ्री वीडियो एडिटर — Shorts और Reels के लिए बेहतरीन।",
       "links": [
        {
         "label": "App",
         "urls": [
          "https://play.google.com/store/apps/details?id=com.google.android.apps.youtube.creator"
         ]
        },
        {
         "label": "Tutorial",
         "urls": [
          "https://www.youtube.com/results?search_query=youtube+create+app+video+editing+tutorial"
         ]
        }
       ]
      },
      {
       "title": "Snapseed",
       "desc": "गूगल का फ्री प्रो-लेवल फोटो एडिटर — RAW एडिटिंग, कर्व्स और बहुत कुछ करता है।",
       "links": [
        {
         "label": "App",
         "urls": [
          "https://play.google.com/store/apps/details?id=com.niksoftware.snapseed"
         ]
        },
        {
         "label": "Tutorial",
         "urls": [
          "https://www.youtube.com/results?search_query=snapseed+photo+editing+tutorial"
         ]
        }
       ]
      },
      {
       "title": "Remove.bg",
       "desc": "इमेजेस से बैकग्राउंड हटाने के लिए एकदम क्लीन और सटीक AI टूल।",
       "links": [
        {
         "label": "Tool Name",
         "urls": [
          "http://remove.bg"
         ]
        },
        {
         "label": "Web",
         "urls": [
          "https://www.remove.bg"
         ]
        },
        {
         "label": "App",
         "urls": [
          "https://play.google.com/store/apps/details?id=bg.remove.android"
         ]
        },
        {
         "label": "Tutorial",
         "urls": [
          "https://www.youtube.com/results?search_query=remove.bg+tutorial"
         ]
        }
       ]
      },
      {
       "title": "Pixelcut",
       "desc": "प्रोडक्ट फोटोग्राफी और सोशल मीडिया के लिए AI-पावर्ड इमेज एडिटिंग ऐप।",
       "links": [
        {
         "label": "Web",
         "urls": [
          "https://www.pixelcut.ai"
         ]
        },
        {
         "label": "App",
         "urls": [
          "https://play.google.com/store/apps/details?id=com.circular.pixelcut"
         ]
        },
        {
         "label": "Tutorial",
         "urls": [
          "https://www.youtube.com/results?search_query=pixelcut+app+tutorial"
         ]
        }
       ]
      },
      {
       "title": "Freepik",
       "desc": "All-in-One AI Creative Suite — डिज़ाइन, वेक्टर्स और इमेजेस के लिए।",
       "links": [
        {
         "label": "Web",
         "urls": [
          "https://www.freepik.com"
         ]
        },
        {
         "label": "App",
         "urls": [
          "https://play.google.com/store/apps/details?id=com.freepik.app"
         ]
        },
        {
         "label": "Tutorial",
         "urls": [
          "https://www.youtube.com/results?search_query=how+to+use+freepik+tutorial"
         ]
        }
       ]
      },
      {
       "title": "Reduce Images",
       "desc": "फ्री ऑनलाइन टूल — इमेजेस का साइज़ (KB) कम करो बिना क्वालिटी खोए।",
       "links": [
        {
         "label": "Web",
         "urls": [
          "https://www.reduceimages.com/"
         ]
        },
        {
         "label": "Tutorial",
         "urls": [
          "https://www.youtube.com/results?search_query=reduce+image+size+online+tutorial"
         ]
        }
       ]
      },
      {
       "title": "PDF Resizer",
       "desc": "मल्टी-पेज PDFs का साइज़ और डाइमेंशन्स एडजस्ट करने का फ्री टूल।",
       "links": [
        {
         "label": "Web",
         "urls": [
          "https://pdfresizer.com/multipage"
         ]
        },
        {
         "label": "Tutorial",
         "urls": [
          "https://www.youtube.com/results?search_query=pdf+resizer+multipage+tutorial"
         ]
        }
       ]
      }
     ]
    },
    {
     "title": "Research — Learning & research utilities, communities, and tracking",
     "items": [
      {
       "title": "Library Genesis",
       "desc": "दुनिया की लगभग हर पेड किताब और रिसर्च पेपर फ्री में डाउनलोड करने का साइट।",
       "links": [
        {
         "label": "Web",
         "urls": [
          "https://libgen.li"
         ]
        },
        {
         "label": "Tutorial",
         "urls": [
          "https://www.youtube.com/results?search_query=how+to+use+libgen+library+genesis"
         ]
        }
       ]
      },
      {
       "title": "Anna's Archive",
       "desc": "दुनिया की सभी शैडो लाइब्रेरीज़ (LibGen, Sci-Hub, Z-Library) का यूनिफाइड सर्च इंजन।",
       "links": [
        {
         "label": "Web",
         "urls": [
          "https://shadowlibraries.github.io/DirectDownloads/AnnasArchive/"
         ]
        },
        {
         "label": "Tutorial",
         "urls": [
          "https://en.wikipedia.org/wiki/Shadow_library"
         ]
        }
       ]
      },
      {
       "title": "Veritasium",
       "desc": "डीप साइंस और लॉजिक को डिकोड करने वाले दुनिया के बेस्ट एजुकेशन YouTubers में से एक।",
       "links": [
        {
         "label": "Web",
         "urls": [
          "https://www.veritasium.com"
         ]
        },
        {
         "label": "Tutorial",
         "urls": [
          "https://www.youtube.com/@veritasium"
         ]
        }
       ]
      },
      {
       "title": "Telegram",
       "desc": "भारी फाइल्स भेजना, ओपन-सोर्स कम्युनिटी चैनल्स और सीक्रेट चैट्स — बेहद सिक्योर।",
       "links": [
        {
         "label": "Web",
         "urls": [
          "https://web.telegram.org/"
         ]
        },
        {
         "label": "App",
         "urls": [
          "https://play.google.com/store/apps/details?id=org.telegram.messenger"
         ]
        },
        {
         "label": "Tutorial",
         "urls": [
          "https://www.youtube.com/results?search_query=telegram+advanced+features+tutorial"
         ]
        }
       ]
      },
      {
       "title": "Product Hunt",
       "desc": "दुनिया भर में रोज़ लॉन्च होने वाले नए ऐप्स और स्टार्टअप्स खोजने की जगह।",
       "links": [
        {
         "label": "Web",
         "urls": [
          "https://www.producthunt.com"
         ]
        },
        {
         "label": "App",
         "urls": [
          "https://play.google.com/store/apps/details?id=com.producthunt.app"
         ]
        },
        {
         "label": "Tutorial",
         "urls": [
          "https://www.youtube.com/results?search_query=how+to+use+product+hunt"
         ]
        }
       ]
      },
      {
       "title": "SocialBlade",
       "desc": "किसी भी YouTube या Instagram अकाउंट के analytics और असली ग्रोथ ट्रैक करने की साइट।",
       "links": [
        {
         "label": "Web",
         "urls": [
          "https://socialblade.com"
         ]
        },
        {
         "label": "Tutorial",
         "urls": [
          "https://www.youtube.com/results?search_query=how+to+use+social+blade+analytics"
         ]
        }
       ]
      },
      {
       "title": "91mobiles",
       "desc": "मोबाइल और गैजेट रिसर्च के लिए भारत की सबसे बड़ी टेक वेबसाइट।",
       "links": [
        {
         "label": "Web",
         "urls": [
          "https://www.91mobiles.com"
         ]
        },
        {
         "label": "Tutorial",
         "urls": [
          "https://www.youtube.com/results?search_query=how+to+compare+phones+on+91mobiles"
         ]
        }
       ]
      }
     ]
    },
    {
     "title": "Dev & Design — Build, code, design — everything with quick tutorials",
     "items": [
      {
       "title": "Google Colab",
       "desc": "ब्राउज़र में ही Python और Machine Learning कोड रन करने का फ्री क्लाउड टूल।",
       "links": [
        {
         "label": "Web",
         "urls": [
          "https://colab.research.google.com"
         ]
        },
        {
         "label": "Tutorial",
         "urls": [
          "https://www.youtube.com/results?search_query=google+colab+tutorial+for+beginners"
         ]
        }
       ]
      }
     ]
    }
   ]
  }
 ]
};
